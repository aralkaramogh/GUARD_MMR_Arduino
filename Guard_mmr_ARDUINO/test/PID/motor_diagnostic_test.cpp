/**
 * ============================================================================
 * COMPREHENSIVE MOTOR DIAGNOSTIC TEST
 * ============================================================================
 * 
 * Tests each motor individually to identify control issues
 * Measures RPM, PWM output, and diagnoses spindown behavior
 * 
 * Commands:
...
bool pidEnabled = false;  // Start with direct PWM control
...
void printHeader() {
  Serial.println("\n====================================================");
  Serial.println("     MOTOR DIAGNOSTIC TEST - DETAILED ANALYSIS");
  Serial.println("====================================================\n");
}
...
void printDetailedStatus() {
  ...
  Serial.println("Motor | Target | Actual | PWM | Pulses | Integral");
  Serial.println("------|--------|--------|-----|--------|----------");
  ...
  for (int i = 0; i < MOTOR_COUNT; i++) {
    ...
    // Pulse Count
    Serial.print(speedPulseCounts[i]);
    Serial.print(" | ");
    
    // Integral
    Serial.println(motorPID[i].integral, 2);
  }
...
void printQuickStatus() {
  ...
  Serial.print(" Pulses:");
  Serial.print(speedPulseCounts[selectedMotor]);
  Serial.print(" Int:");
  Serial.println(motorPID[selectedMotor].integral, 2);
}
...
void printHelp() {
  ...
  Serial.println("  T       Toggle PID mode (currently: ");
...
void setup() {
  ...
  // Setup ISR for motor 1 speed feedback
  ...
  Serial.println("  [OK] Speed feedback polling enabled (M2-M4)");
  Serial.println("  [OK] Interrupt enabled (M1 on pin 50)");
