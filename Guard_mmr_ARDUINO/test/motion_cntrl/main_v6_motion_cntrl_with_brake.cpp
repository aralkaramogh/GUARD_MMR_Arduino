/**
 * Skid-steer motor controller for Arduino Uno + JKBLD300 drivers with BRAKE control.
 *
 * Overview of key responsibilities and functions:
 * - Pin mapping: PWM, DIR, and BRAKE pins for 4 motors (right-front, left-front,
 *   right-back, left-back).
 * - BRAKE Pin Logic: BRAKE = HIGH (engaged), BRAKE = LOW (disengaged/motor active)
 * - setMotor(index,duty): sets a single motor's direction, PWM, and brake state.
 * - Brake Engagement: Automatically engaged when motor duty is 0, disengaged when moving.
 ...
 * - stopAll(): stops motors and engages all brakes.
 * - resetControl(): stops motors, engages brakes, and resets speeds to 10%.
...
// BRAKE Pins (Any Digital Pin) - HIGH = Brake Engaged, LOW = Brake Disengaged
const int MOTOR_BRAKE_1 = 10;  // Right Front Brake
const int MOTOR_BRAKE_2 = 11;  // Left Front Brake (note: 11 is PWM capable but using as digital)
const int MOTOR_BRAKE_3 = 2;   // Right Back Brake (Speed feedback was on 2, now using for brake)
const int MOTOR_BRAKE_4 = 14;  // Left Back Brake (A0)
...
// Brake state tracking
bool brakeEngaged[4] = {true, true, true, true};  // Initially all brakes engaged
...
void setBrake(int index, bool engaged) {
  if (index < 0 || index >= MOTOR_COUNT) return;
  
  brakeEngaged[index] = engaged;
  // HIGH = Brake Engaged, LOW = Brake Disengaged
  digitalWrite(BRAKE_PINS[index], engaged ? HIGH : LOW);
}

void engageAllBrakes() {
  for (int i = 0; i < MOTOR_COUNT; ++i) {
    setBrake(i, true);
  }
}

void disengageAllBrakes() {
  for (int i = 0; i < MOTOR_COUNT; ++i) {
    setBrake(i, false);
  }
}
...
void setMotor(int index, int dutyPercent) {
  ...
  // Brake Logic:
  // - If duty is 0 (stopped), engage brake
  // - If duty is non-zero (moving), disengage brake
  if (duty == 0) {
    setBrake(index, true);   // Engage brake when stopped
  } else {
    setBrake(index, false);  // Disengage brake when moving
  }
...
void forward(int speedPercent) {
  ...
  Serial.print("Cmd: Forward | Speed: "); Serial.print(speedPercent); Serial.println(" | Brakes: Disengaged");
}
...
void backward(int speedPercent) {
  ...
  setLeftMotorsAll(-speedPercent);
  setRightMotorsAll(-speedPercent);
  Serial.print("Cmd: Backward | Speed: "); Serial.print(speedPercent); Serial.println(" | Brakes: Disengaged");
}
...
void turnRight(int speedPercent) {
  ...
  setLeftMotorsAll(speedPercent);
  setRightMotorsAll(-speedPercent);
  Serial.print("Cmd: Right | Speed: "); Serial.print(speedPercent); Serial.println(" | Brakes: Disengaged");
}
...
void turnLeft(int speedPercent) {
  ...
  setLeftMotorsAll(-speedPercent);
  setRightMotorsAll(speedPercent);
  Serial.print("Cmd: Left | Speed: "); Serial.print(speedPercent); Serial.println(" | Brakes: Disengaged");
}

void stopAll() {
  setAllMotors(0);  // setMotor automatically engages brakes when duty is 0
  Serial.println("Cmd: Stop | Brakes: Engaged");
}

void resetControl() {
  stopAll();  // Stops all motors and engages all brakes
  forwardBackwardSpeed = 10;
  // Reset both forward/backward and turning speeds to defaults
  turningSpeed = 10;
  Serial.println("Reset: All motors stopped, brakes engaged, speeds reset to 10.");
}
...
void setup() {
  ...
  for (int i = 0; i < MOTOR_COUNT; ++i) {
    ...
    digitalWrite(BRAKE_PINS[i], HIGH); // Start with brakes engaged (HIGH)
  }
  
  pinMode(LED_PIN, OUTPUT);
  digitalWrite(LED_PIN, LOW); 

  Serial.println("=== Arduino Uno Motor Control Ready (with Brake) ===");
  Serial.println("Brake Pin Logic: HIGH = Engaged, LOW = Disengaged");
  Serial.println("All brakes are initially ENGAGED for safety.");
  Serial.println("");
  ...
  Serial.println("  H - Reset (stop, engage brakes, Fwd/Back -> 10, Turning -> 10)");
  ...
  Serial.println("All brakes currently: ENGAGED");
}
