...
void loop()
{
  if (Serial.available() > 0)
  {
    ...
    if (cmd == 'a')
    {
      if (Serial.available() >= 1)
      {
        ...
        if (pwmVal >= 0 && pwmVal <= 255)
        {
          ...
          Serial.print("All motors set to PWM ");
          Serial.println(pwmVal);
        }
      }
      return;
    ...
    case 'r':
      turnRatioPercent += 5;
      if (turnRatioPercent > 100)
        turnRatioPercent = 100;
